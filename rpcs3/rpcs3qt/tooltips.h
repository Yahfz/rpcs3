...
public:
	...
	const struct settings
	{
		...
		const QString custom_xfloat             = tr("Shows the XFloat accuracy selected for each affected SPU instruction.\nPreset values are read-only. Select Custom XFloat to edit each instruction independently or initialize it from an existing preset.\nOnly accuracy levels with distinct implementations are offered for each instruction. Use custom combinations only for testing because they can cause game-specific issues.");
