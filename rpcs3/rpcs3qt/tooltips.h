...
public:
	...
	const struct settings
	{
		...
		const QString custom_xfloat             = tr("Shows the XFloat accuracy selected for each affected SPU instruction.\nPreset values are read-only. Select Custom XFloat to edit each instruction independently.\nSome accuracy levels currently share the same implementation for individual instructions. Use custom combinations only for testing because they can cause game-specific issues.");
